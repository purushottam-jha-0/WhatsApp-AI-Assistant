...
// // 2. Position the overlay near the input box
// function updateOverlayPosition(inputBox) {
//     if (!suggestionOverlay) createOverlay();
//     const rect = inputBox.getBoundingClientRect();
//     // Position slightly above the input box
//     suggestionOverlay.style.left = rect.left + 'px';
//     suggestionOverlay.style.top = (rect.top - 30) + 'px'; 
//     suggestionOverlay.style.width = rect.width + 'px';
// }

// // 3. Listen for typing in WhatsApp
// document.addEventListener('input', (e) => {
//     const inputBox = e.target.closest('div[contenteditable="true"]');
//     if (!inputBox) return;

//     suggestionOverlay.style.display = 'none'; // Hide old suggestion
    
//     // Debounce: Wait 1 second after typing stops to fetch AI
//     lastInputTime = Date.now();
//     setTimeout(() => {
//         if (Date.now() - lastInputTime >= 1000) {
//             fetchSuggestion(inputBox.innerText);
//             updateOverlayPosition(inputBox);
//         }
//     }, 1000);
// });

// // 4. Handle "Tab" key to accept suggestion [cite: 16]
// document.addEventListener('keydown', (e) => {
//     if (e.key === 'Tab' && suggestionOverlay && suggestionOverlay.style.display !== 'none') {
...
//         // Move cursor to end (hacky but works for PoC)
...
//     if (text.length < 5) return; // Don't suggest for very short text
...
//             { type: 'GET_COMPLETION', text: text, tone: data.tone || 'Casual' },
//             (response) => {
//                 if (response && response.suggestion) {
//                     currentSuggestion = response.suggestion;
//                     suggestionOverlay.textContent = "Tab to complete: " + currentSuggestion;
